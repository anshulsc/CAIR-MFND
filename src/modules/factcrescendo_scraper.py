...
def get_latest_article_links(count=10):
    """Fetch latest fact-check article links from FactCrescendo English"""
    main_page_url = 'https://english.factcrescendo.com/'
    ...
    try:
        ...
        # FactCrescendo uses article links in various formats
        # Look for links that contain year format (e.g., /2025/)
        all_links = soup.find_all('a', href=True)
        
        for link in all_links:
            href = link.get('href')
            if href:
                # Check if it's an article URL (contains year pattern)
                if '/2025/' in href or '/2024/' in href:
                    ...
                    if href not in seen_urls and 'category' not in href and 'author' not in href:
                        seen_urls.add(href)
        ...
        worker_logger.error(f"Error fetching the main FactCrescendo page: {e}")
        return []

def scrape_and_save_article(url: str):
    """Scrape a single FactCrescendo article and save its content"""
    ...
    try:
        ...
        # Extract title - FactCrescendo uses h1 or article title
        title_elem = soup.select_one('h1.entry-title') or soup.select_one('h1') or soup.select_one('article h1')
        caption = title_elem.text.strip() if title_elem else "Untitled"

        # Extract date - look for time element or date meta tags
        date_elem = (
        ...
        if date_elem:
            timestamp = date_elem.get('datetime', date_elem.text.strip())
        else:
            # Try to extract from URL (e.g., /2025/12/11/)
            import re
            date_match = re.search(r'/(\d{4}/\d{2}/\d{2})/', url)
            timestamp = date_match.group(1) if date_match else "N/A"

        evidence_id = f"factcrescendo_{uuid.uuid4().hex[:10]}"
        save_dir = EVIDENCE_DB_DIR / evidence_id
        save_dir.mkdir(exist_ok=True)
        
        # Extract the main featured image
        img_elem = (
            soup.select_one('article img') or 
            soup.select_one('.wp-post-image') or
            soup.select_one('img[class*="featured"]') or
...
def run_factcrescendo_pipeline(count=10):
    """Main pipeline to scrape and index FactCrescendo articles"""
    worker_logger.info("--- Starting FactCrescendo Scraper & Indexer Pipeline ---")
    ...
    if not latest_links:
        return {
            ...
            "message": "Could not retrieve any article links from FactCrescendo."
        }

    # Get existing FactCrescendo items from the database
    existing_items_db = collection.get(where={"source": "factcrescendo.com"}, include=["metadatas"])
    url_to_data_map = {}
    ...
    for link in latest_links:
        if link in url_to_data_map:
            ...
            if article_data:
                ...
                worker_logger.info(f"Indexing new FactCrescendo evidence: {article_data['evidence_id']}")
...
if __name__ == "__main__":
    ...
    print("Testing FactCrescendo Scraper...")
    print("=" * 60)
    
    result = run_factcrescendo_pipeline()
